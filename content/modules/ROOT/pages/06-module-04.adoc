...
[[exercise-5]]
== Exercise 5: Backup and Restore Virtual Machines
...
image::06-module-04-bnr-vm-backup-policy.png[VM Back up Policy,link=self,window=blank,width=700,title="VM Backup Policy"]
...
image::06-module-04-bnr-vm-backup-set-policy.png[VM Set backup policy,link=self,window=blank,width=700,title="VM Set backup policy"]

Wait for backup to show as Completed under Backup status. 

image::06-module-04-bnr-vm-backup-complete.png[VM Backup Complete,link=self,window=blank,width=700,title="VM Backup Complete"]
...
image::06-module-04-bnr-vm-backup-jobs.png[VM Backup Jobs,link=self,window=blank,width=700,title="VM Backup Jobs"]
...
image::06-module-04-bnr-vm-backup-summary.png[VM Backup Summary,link=self,window=blank,width=700,title="VM Backup Summary"]
...
image::06-module-04-bnr-vm-backed-apps.png[VM Backed up Applications,link=self,window=blank,width=700,title="VM Backed up Applications"]

Click on the Restore (A) button to begin the restore process.

image::06-module-04-bnr-vm-restore.png[VM Restore,link=self,window=blank,width=700,title="VM Restore"]

The wizard presents the choice to restore to the same cluster or to a different cluster, if you have a hub-spoke setup. Open the drop-down list associated with the “Target cluster” field and select the This cluster (A) item from the list.

image::06-module-04-bnr-vm-restore-select-dest.png[VM Select Destination,link=self,window=blank,width=700,title="VM Restore select destination"]
...
image::06-module-04-bnr-vm-restore-project.png[VM Restore Project,link=self,window=blank,width=700,title="VM Restore Project"]
...
image::06-module-04-bnr-vm-restore-select.png[VM Restore Select,link=self,window=blank,width=700,title="VM Restore Select"]

Review the Summary (A) details for OpenShift Project and Restore point for accuracy. Click the Restore (B) button.

image::06-module-04-bnr-vm-restore-button.png[VM Restore Button,link=self,window=blank,width=700,title="VM Restore Button"]

Click the Restore (A) button again to confirm the start of the restore process.

image::06-module-04-bnr-vm-confirm-restore.png[VM Confirm Restore,link=self,window=blank,width=700,title="VM Comfirm Restore"]

Click on the View job details (A) button to open the vmbackup restore details page.

image::06-module-04-bnr-vm-job-details.png[VM Job Details,link=self,window=blank,width=700,title="VM Job Details"]

Wait for the restore backup job to show Completed. 

image::06-module-04-bnr-vm-restore-complete.png[VM Restore Complete,link=self,window=blank,width=700,title="VM Restore Complete"]

Return to the OpenShift GUI. Navigate to the VirtualMachines screen by clicking on the Virtualization (A) menu item on the left-hand side navigation pane and selecting the VirtualMachines (B) sub-item. Use the Project selector to change to the vmrestore (C) project. Notice that the VirtualMachine has been restored to a new namespace and is in a Running Status.

image::06-module-04-bnr-vm-list-restored.png[List Restored VM,link=self,window=blank,width=700,title="List Restored VM"]
