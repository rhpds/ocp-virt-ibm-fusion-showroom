...
* OpenShift console in a tab on your right or if you prefer in a separate window: {openshift_console_url}[window=blank].
...
. If you see the *Welcome to OpenShift Virtualization* screen, check the checkbox next to *Do not show this again* or simply close the window by clicking the `X`.
...
. In the OpenShift console, switch to the *Administrator* perspective and navigate to **Storage** > **Data Foundation** in the left navigation menu.
+
...
. Using single sign on you are already logged into the IBM Fusion Web Console as `{openshift_cluster_admin_username}`.
...
. For *Project* open the drop-down, click *Show default projects* and select the `default` project.
. Select **Red Hat Enterprise Linux 9 VM** as illustrated below.
+
...
. Scroll down and select a *U series small* instance type under *Select InstanceType*.
