...
. Verify the VM deployment is complete (repeat until you see the expected status)
+
[source,role="execute"]
----
...
. Watch the VMI object recreate (old one deleted, new one appears). Hit `Ctrl-C` once you see the new VMI phase as `Running`.
+
[source,role="execute"]
----
...
. Access the VM (use `cloud-user` and `redhat123` as the credentials)
+
[source,role="execute"]
----
...
. Connect back to your VM (`cloud-user`, `redhat123`, `Ctrl-5` to disconnect)
+
[source,role="execute"]
----
virtctl console rhel9-webserver
