...
rhel9-webserver-rootdisk   Bound    pvc-<unique_uuid>                          30Gi       RWX            ocs-storagecluster-ceph-rbd-virtualization   <unset>                 <age>
----
...
----
+
WARNING: It will take some time for the VM to start and the `cloud-init` step to complete. Once no messages are displayed the VM is ready
...
----
+
TIP: Hit <CR> to get to the login prompt. The VM username is `cloud-user` and the password `redhat123`
+
Expected output of your `curl` command
+
----
