...
oc get pods -n vmlab-student -l kubevirt.io/vm=rhel9-webserver
...
curl -L -o virtctl \
  https://$(oc get route virtctl-downloadserver -n openshift-cnv \
  -o jsonpath='{.spec.host}')/amd64/linux/virtctl.tar.gz | tar xz
