...
. Copy the publick key to your clipboard
+
[source, bash]
----
cat ~/.ssh/id_rsa.pub
...
. Connect to you VM using the console 
+
[source, bash]
----
virtctl console rhel9-webserver -n vmlab-student
----
+
NOTE: Use the same `cloud-user` username and `redhat123` password as earlier

. Update `/home/cloud-user/.ssh/authorized_keys` using the value from your clipboard
+
WARNING: In a real environment you use the `cloud-init` section of the VM template to automatically drop your public key into the VM.
+
NOTE: When done disconnect from the VM console using `exit` followed by `CTRL+5`.
